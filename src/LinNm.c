...
Std_ReturnType LinNm_NetworkRequest(NetworkHandleType NetworkHandle) {	
	...
	if (LINNM_DEV_ERROR_DETECT == STD_ON){
		...
		else if (LinNm_Internal.InitStatus == LINNM_STATUS_UNINIT || ChannelInternal->State == NM_STATE_UNINIT){
			/*[SWS_LinNm_00037]*/
			error_code = LINNM_E_UNINIT;
			status = E_NOT_OK;
		}
...
Std_ReturnType LinNm_CheckRemoteSleepIndication(NetworkHandleType NetworkHandle, boolean* nmRemoteSleepIndPtr) {
    ...
    if (LINNM_DEV_ERROR_DETECT == STD_ON) {
        ...
        /*[SWS_LinNm_00029]*/
        else if (nmRemoteSleepIndPtr == NULL) {
            error_code |= LINNM_E_PARAM_POINTER;
        }
